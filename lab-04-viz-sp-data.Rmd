...
1.  What are the dimensions of the Denny's dataset?
    ...
    ***There are 1643 rows and 6 columns.
    Each row represents the data for one denny's restaurant.
...
2.  What are the dimensions of the La Quinta's dataset?
    ...
    ***What are the variables?
    ...
    Each row represents the data for on La Quintas Inns.
...
3.  Take a look at the websites that the data come from (linked above).
    ...
    What about Denny's?
    ***La Quinta hotels are mostly located in the Americas but also have locations in other parts of the world.
...
4.  Now take a look at the data.
  ...
  ***We can use the state variable to deterimind whether a location is in the US. Specifically by comparing the state listed to a database of states in the US.***
...
9.  Which states have the most and fewest Denny's locations? What about La Quinta? Is this surprising? Why or why not? ***The state with the most Denny's locations is CA, the least is DE. The state with the most laquinta is TX , the least is AG. This is not surprising to me because the bigger states have more and the smaller states have less***
...
***DC has the most Denny's per thousand square miles. Rhode Island has the most la Quinata per thousand square miles***
```{r}
dennys <-dennys %>%
  count(state) %>%
  inner_join(states, by = c("state" = "abbreviation")) %>%
...
11. Filter the data for observations in North Carolina only, and recreate the plot.
    ...
    ***Visually this joke doesn't appear to hold up here***
```{r}
dn_lq %>%
  filter(state == "NC") %>%
...
12. Now filter the data for observations in Texas only, and recreate the plot, with an appropriate `alpha` level.
    Visually, does Mitch Hedberg's joke appear to hold here?
    ***Visually the joke does apprear to hold up here***
```{r}
dn_lq %>%
  filter(state == "TX") %>%
ggplot(mapping = aes(x = longitude, y = latitude, color = establishment)) +
